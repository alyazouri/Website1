...
export type SensParams = {
  ...
  proProfile?: string; // aggressive | balanced | competitive | headshot_pro
};
...
export function computeSensitivity(p: SensParams): Sens {
  ...
  // ──── COMPUTE MULTIPLIERS ────
  const cqcMul = dM * fM * sC * gOff * profCQC;
  const scopeNear = dM * fM * sS * adsF * profNear;
  const scopeFar = dM * fM * sS * scopeStab * profFar;
  const gBase = dG * fM * sG * profGyro;
  const gScopeFar = gBase * scopeStab * gyroHComp * profGyroFar;
